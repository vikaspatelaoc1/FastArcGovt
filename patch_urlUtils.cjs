...
const path = 'src/utils/urlUtils.ts';
let code = fs.readFileSync(path, 'utf8');

// We want to rewrite normalizeExternalUrl to properly add www. to naked domains
const replacement = `export function normalizeExternalUrl(url?: string): string {
...
code = code.replace(/export function normalizeExternalUrl\([\s\S]*?\}\n\}/, replacement);
fs.writeFileSync(path, code);
console.log('Patched urlUtils');
