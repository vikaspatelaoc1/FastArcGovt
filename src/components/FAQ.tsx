  ...
  return (
    <div className="w-full max-w-4xl px-4 sm:px-6 lg:px-8 py-8 mt-4 border-t border-slate-200 dark:border-slate-800 mr-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-8 h-8 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 rounded-lg flex items-center justify-center shadow-sm">
          <HelpCircle className="w-4 h-4" />
        </div>
        <div>
          <h2 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight">Frequently Asked Questions</h2>
          <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mt-0.5">Common queries about government job updates and our portal</p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-start gap-2">
        {faqs.map((faq, index) => {
          ...
          return (
            <div 
              key={index} 
              className={`border transition-all duration-200 rounded-lg overflow-hidden w-fit max-w-full ${isOpen ? 'bg-white dark:bg-slate-800 border-indigo-200 dark:border-indigo-800/50 shadow-sm' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700'}`}
            >
              <button
                onClick={() => toggleFAQ(index)}
                className="text-left px-3 py-2 flex items-center gap-1.5 focus:outline-none cursor-pointer w-full"
              >
                <span className={`font-bold text-xs ${isOpen ? 'text-indigo-700 dark:text-indigo-400' : 'text-slate-800 dark:text-slate-200'}`}>
                  {faq.question}
                </span>
                <ChevronDown className={`w-3.5 h-3.5 shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`} />
              </button>
              
              <div 
                className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0 hidden'}`}
              >
                <div className="px-3 pb-2 text-[11px] font-medium text-slate-600 dark:text-slate-400 leading-relaxed max-w-[500px]">
                  {faq.answer}
                </div>
              </div>
            </div>
